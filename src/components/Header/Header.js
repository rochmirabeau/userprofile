...
export function SearchButton (props){
  return(
          <svg fill="#ffffff" height="18" viewBox="0 0 24 24" width="18" xmlns="http://www.w3.org/2000/svg" className="u-float-right header-icon">
...
export function HeaderStats (props){
  ...
  return(
      ...
      <div className="stats">
        {
          stats.map(stat => <span key={ stat.title } className="stat-module"> { stat.title } <span> { stat.number }</span></span>)
        }
      </div>
  )
}
